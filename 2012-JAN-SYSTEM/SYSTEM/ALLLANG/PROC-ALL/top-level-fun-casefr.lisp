...
;                |--> postagger
;             |--> parse-single-f (tut)
;             |         (input: .tb and .prs file names; reads from .tb; writes on .prs)
...
(defun ana-and-tag-and-parse ()
 ...
 (let (answ xinput)
    ...
    (cond ((eq answ 'n)
            (format t "~% Name of the input file containing the raw text (between quotation marks)? ~%")
            (format t "     Home directory: *HOME-DIR*/../DATI ~%")
            (setq xinput (build-subdir-file-name (read) "../DATI/"))
...
; *************************************************************************
(defun ana-and-tag-and-parse-and-seminterp ()
 (declare (special *ONTOLOGY-CACHE* *ONTO-CACHE-FILE* *TREE-FORMAT* *PRINT-LEV* 
                   *HOME-DIR* *SYSTEM-CONTEXT* *TURN-TO-TALK*))
 (let (sentence tagres parseres new-tree semrepr tempparse inpsource savhome fpathname
  ...
  (cond ((eq trace? 'y) (setq trace? t))
        (t (setq trace? nil)))
  (cond ((eq inpsource 'f)
     ;      (setq savhome *HOME-DIR*)
     ;      (setq *HOME-DIR* "/home/lesmo/TULE/DATI/ATLAS/")
...
; *** linecount used in tokenize
   ...
   (with-open-file (portin filename
     ...
     (with-open-file (outputport xoutpt
                                 :direction :output
                                 :if-exists :overwrite
                                 :if-does-not-exist :create)
     (with-open-file (outputport2 xoutpt2
                                 :direction :output
                                 :if-exists :overwrite
                                 :if-does-not-exist :create)
     (with-open-file (outputport3 xoutpt3
                                 :direction :output
                                 :if-exists :overwrite
                                 :if-does-not-exist :create)
     (with-open-file (outputport4 xoutpt4
                                 :direction :output
                                 :if-exists :overwrite
                                 :if-does-not-exist :create)
			  ...
			  (setq one-phrase
				   ...
				   (append (butlast act-phrase)
					   (choose-number-interp (ult act-phrase)))
                                   (third one-phrase)))
	 	          (setq next-phrase (append next-phrase act-phrase)))
	 	        (t (setq next-phrase (append next-phrase act-phrase))))
  ...
  ;   (break "")
     ...
     ;     specified in the file Header
	 	 (setq postag-result 
			 	 ...
			 	 outputport outputport2 outputport3 outputport4))
...
end-sent sigla-continue)
(declare (special element errore doppia flag graf inpt
    scan-element rem-line *ANALYS-CONTEXT*))
; *** la funzione "tokenize", richiamata nella do sotto, analizza la stringa di
;     input riga per riga e termina l'esecuzione quando trova un separatore
;     di frase, restituendo la tokenizzazione della frase. Il buffer rem-line
;     contiene eventuali porzioni di linea non analizzate nella frase
;     precedente, da cui si deve ricominciare nella frase successiva
(setq rem-line nil)
(do* ((one-phrase (tokenize input-string) (tokenize rest-sent))
   (tok-data (second one-phrase) (second one-phrase))
   (rest-sent (third one-phrase) (third one-phrase))
   (stop nil (first one-phrase)))
 ((and (eq stop 's-eof) (null (second one-phrase)))
     (reverse tagresult))
; (setq one-phrase (tokenize input-string))
; (setq end-sent (first one-phrase))
; ****************************
;     THE NEXT PART TO DECIDE IF TWO CANDIDATE SENTENCES HAVE TO BE TREATED AS A
;     SINGLE ONE
; ****************************
...
;     longer than 300 words).
       ((or (and in-parenth
			 (neq 's-eof end-sent)
			 (neq 'closed inside-par)
		         (not (> (+ (length next-phrase) (length tok-data))
                                 300)))
		    (and (neq 's-eof (first one-phrase))
			 (eq 'open inside-par)
		         (not (> (+ (length next-phrase) (length tok-data))
...
;     terminated.
    (cond ((and sent-contin
	     ...
	     (setq rest-sent (third one-phrase)))
