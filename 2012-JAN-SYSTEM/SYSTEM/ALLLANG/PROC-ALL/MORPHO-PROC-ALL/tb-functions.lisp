...
; *****************************************************************
(defun is-newtb-possperson (val)
   (memq val '(p-1 p-2 p-3 p-allval)))

; *****************************************************************
(defun is-newtb-possgender (val)
   (memq val '(p-m p-f p-n p-allval)))

; *****************************************************************
(defun is-newtb-possnumber (val)
   (memq val '(p-sing p-pl p-allval)))
...
; *******************************************************************
; *** possessor person (e.g. in "our": p-1)
(defun get-newtb-possperson (wdata)
 (let ((word (get-newtb-syntinfo wdata)))
  (cond ((eq (third word) 'poss)
          (cond ((is-newtb-possperson (fifth word)) (fifth word))
                ((is-newtb-possperson (sixth word)) (sixth word))
                ((is-newtb-possperson (seventh word)) (seventh word))
                ((is-newtb-possperson (eighth word)) (eighth word))
                (t nil)))
        (t nil))))

; *******************************************************************
; *** possessor gender (e.g. in "her": p-f)
(defun get-newtb-possgender (wdata)
 (let ((word (get-newtb-syntinfo wdata)))
  (cond ((eq (third word) 'poss)
          (cond ((is-newtb-possgender (fifth word)) (fifth word))
                ((is-newtb-possgender (sixth word)) (sixth word))
                ((is-newtb-possgender (seventh word)) (seventh word))
                ((is-newtb-possgender (eighth word)) (eighth word))
                (t nil)))
        (t nil))))

; *******************************************************************
; *** possessor number (e.g. in "nostro": the number is SING, but the possessor number is P-PL)
(defun get-newtb-possnumber (wdata)
 (let ((word (get-newtb-syntinfo wdata)))
  (cond ((eq (third word) 'poss)
          (cond ((is-newtb-possnumber (fifth word)) (fifth word))
                ((is-newtb-possnumber (sixth word)) (sixth word))
                ((is-newtb-possnumber (seventh word)) (seventh word))
                ((is-newtb-possnumber (eighth word)) (eighth word))
                (t nil)))
        (t nil))))
...
; *** it returns the value of the component (a number)
 (let ((word (get-newtb-syntinfo wdata)))
  (cond ((memq (second word) '(NUM DATE))
           (third word))
        ((and (eq (second word) 'ADJ) (eq (third word) 'ORDIN))
           (sixth word))
        ((and (eq (second word) 'PRON) (eq (third word) 'ORDIN))
           (setq value (sixth word))
           (cond ((or (numberp value) (eq value '£last)) value)
                 (t nil)))
	(t (exception 'generic-error
...
; ***************************************************************
(defun get-newtb-adjgrade (wdata)
 (let ((word (get-newtb-syntinfo wdata)))
  (cond ((eq 'ADJ (second word))
           (cond ((memq (sixth word) '(SUPERL COMPAR))
                   (sixth word))
                 (t nil)))
	(t (exception 'generic-error
                      "PROC/tb-functions: Adjgrade taken from a non-adjective component")))))
   ...
   (let ((syntdata (get-flatavm-feat-val wdata 'syn)))
       (cond ((null syntdata) wdata)
             (t (replace-flatavm-feat-val wdata 'syn 
                      (replace-flatavm-feat-val syntdata 'type newtype))))))
...
; *******************************************************************
(defun get-flatavm-possnumber (wdata)
  (get-flatavm-feat-val (get-flatavm-syntinfo wdata) 'possnumb))
...
; *******************************************************************
(defun get-flatavm-possgender (wdata)
  (get-flatavm-feat-val (get-flatavm-syntinfo wdata) 'possgend))
...
; *******************************************************************
(defun get-flatavm-possperson (wdata)
  (get-flatavm-feat-val (get-flatavm-syntinfo wdata) 'posspers))
...
; *******************************************************************
(defun get-flatavm-adjgrade (wdata)
; *** it returns the grade (SUPERL, COMPAR or NIL) of an adjective
  (cond ((eq (get-flatavm-categ wdata) 'ADJ)
           (get-flatavm-feat-val (get-flatavm-syntinfo wdata) 'grade))
	(t (exception 'generic-error
                      "PROC/tb-functions: AdjGrade taken from a non-adjectival component (avm)"))))
...
(defun get-synt-possnumber (line)
  (declare (special *TREE-FORMAT*))
  (cond ((eq *TREE-FORMAT* 'tut)
           (get-newtb-possnumber line))
	(t (get-flatavm-possnumber line))))
...
(defun get-synt-possgender (line)
  (declare (special *TREE-FORMAT*))
  (cond ((eq *TREE-FORMAT* 'tut)
           (get-newtb-possgender line))
	(t (get-flatavm-possgender line))))
...
(defun get-synt-possperson (line)
  (declare (special *TREE-FORMAT*))
  (cond ((eq *TREE-FORMAT* 'tut)
           (get-newtb-possperson line))
	(t (get-flatavm-possperson line))))
...
(defun get-synt-adjgrade (line)
  (declare (special *TREE-FORMAT*))
  (cond ((eq *TREE-FORMAT* 'tut)
           (get-newtb-adjgrade line))
	(t (get-flatavm-adjgrade line))))
