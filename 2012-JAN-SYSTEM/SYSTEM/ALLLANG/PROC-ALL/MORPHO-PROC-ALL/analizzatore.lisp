...
;     this is needed for dictionary access
 ...
 ;(format t "analyser-elem; el: ~a~% code-el ~a~% result: ~a~%" el code-el result)
 ;(break "")
          ...
          ;     a pronoun is treated as having an adjectival morphological class
	            ...
	            ((eq catdiz-a 'ADJ)
                      (let ((clcod (second class-codes))
                            (agrade (prendi (without-lemma dizword) 'grade))
                            defaultinf)
                          (cond ((null agrade)
                                   (setq defaultinf '(TYPE QUALIF GRADE POS)))
                                (t (setq defaultinf '(TYPE QUALIF))))
                          (cond ((atom (first clcod))
                                   (setq temp (list (append dizword defaultinf clcod))))
                                (t (setq temp
                                      (mapcar #'(lambda (x) (append dizword defaultinf x))
                                            clcod)))))
