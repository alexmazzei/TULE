...
; *** the "assertion id" is the second element (e.g. the class name)
         ...
         (cond ((memq assert-type '(defconcept relation relconstr))
...
; *** for subclass-of, add the link and its inverse
  ...
  ;     rel-restr refers to relations where one of the arguments is constrained to be a given instance
  ;       They have the standard form of a relation definition
               ...
               ((eq assert-type 'relinstance)
                 ...
                 (cond ((not (is-subclass-of 
                         ...
                         (add-prop-val instance2 ; £Lingotto_Musica_TC is
                                       ...
                                       assert-firstarg))))) 	; &has-office1
  ; *** the next is for constraints on relation arguments. They are similar to KL-ONE 
  ;     "structural descriptions". An example is:
  ;     (relconstr &&exper-prop-val-of (refconc ££to-feel)
  ;                (constrdef (££has-property
  ;                                  (&involved-property &experienced-prop)
  ;                                  (&has-property-value &exper-eval))))
  ;     This specifies that &&exper-prop-val-of is a constraint associated with the concept
  ;     ££to-feel (refconc) and that the definition of the constraint (constrdef) involves
  ;     the roles &experienced-prop and &exper-eval: they must be linked via the ££has-property
  ;     reified relation in such a way that the filler of the ££to-feel experienced property
  ;     (&exper-prop) plays the role of &involved-property with respect to evaluation of the
  ;     property (&has-property-value)
               ((eq assert-type 'relconstr)
                 (let ((refconcdef (third nxtassertion))
                       (constrdef (fourth nxtassertion)))
                     (cond ((eq (first refconcdef) 'refconc)
                              (add-prop-val assert-firstarg 'constrained-conc (second refconcdef))
                              (add-prop-val (second refconcdef) 'constraints assert-firstarg))
                           (t (exception 'loadonto-error
                                "PROC/loadfunctions: Missing refconc in a relconstr" 
                                assert-firstarg)))
                     (cond ((eq (first constrdef) 'constrdef)
                              (add-prop-val assert-firstarg 'constraint-pred 
                                     (first (second constrdef)))
                              (add-prop-val assert-firstarg 'constraint-roles 
                                     (rest (second constrdef))))
                           (t (exception 'loadonto-error
                                "PROC/loadfunctions: Missing constrdef in a relconstr" 
                                assert-firstarg)))))
               ((eq assert-type 'disjoint-concepts)
; *** disjointness expressions are give as 4-tuples:
;     (disjoint-concepts first-disjoint-concept second-disjoint-concept upperconcept)
;     which has to be read: first-disjoint-concept is dijoint from second-disjoint-concept
;     with respect to upperconcept
                  (let ((assert-firstarg (second nxtassertion))
                        (assert-secondarg (third nxtassertion))
                        (upperconc (fourth nxtassertion)))
                    (cond ((and (member assert-firstarg *ALL-ONT-CLASSES*)
                                (member assert-secondarg *ALL-ONT-CLASSES*)
                                (member upperconc *ALL-ONT-CLASSES*)
                                (is-subclass-of assert-firstarg upperconc)
                                (is-subclass-of assert-secondarg upperconc))
                              (add-prop-val assert-firstarg 
                                        'disjoint-from (list assert-secondarg upperconc))
                              (add-prop-val assert-secondarg 'disjoint-from 
                                        (list assert-firstarg upperconc)))
                           (t (exception 'loadonto-error
                                "PROC/loadfunctions: Wrong disjointness specification" 
                                assert-firstarg assert-secondarg)))))
               (t (exception 'loadonto-error 
