...
;       22 . (#\. PUNCT) [3;END]
   ...
   ; (format t " Words: ~a~% Links: ~a~% After the insertion of the trace"
   ;                (first data-and-link) (second data-and-link))
   ; (break "")
; *** now, any adverbial or prepositional modifier has been attached to a group
;     depending on newly inserted traces on the basis of "attach-unlinked" rules
;     is moved to the trace
         (setq data-and-link (move-adverbs-and-prep data-and-link))
   ; (format t " Words: ~a~% Links: ~a~% After move adverbs and preps"
   ;                (first data-and-link) (second data-and-link))
   ; (break "")
...
;     In this case, coordlinumb is the line number of the comma
 (declare (special *TREE-FORMAT*))
  ; (break "singsent insert top trace")
...
;       3 giornata (GIORNATA NOUN COMMON F SING) [1.10;VERB-PREDCOMPL+SUBJ]
            ...
            ;  (format t "case 3; nxtline = ~a~%" nxtline)
            ;  (break "")
           ...
           ; *** the two subcases (without or with an initial prep) has been set apart
             ...
             ;  (break "")
                  ...
                  ;           (y ADVB-RMOD-TIME) {for "ancora"} (x.10 VERB-OBJ) {for "una"} ...
                              (setq nxtsentlinks
                                  (cons nxt2link
                                      (cons (make-link traceindex 'VERB-OBJ 'top-traces)
   ...
   ; ****** fifth case *********************************************************
              ...
              ;(format t "case 5; nxtline = ~a~%" nxtline)
              ;(break "")
                     (setq comma-and-art (get-next-comma-art nxtsent nxtsentlinks))
...
(defun make-to-have-trace ()
  (declare (special *TREE-FORMAT*))
  (cond ((eq *TREE-FORMAT* 'tut)
           (list '|t| '(avere verb main ind allval intrans 1 pl) nil 'empty))
        (t `((form #\t)
...
(defun make-to-have-subj-trace ()
  (declare (special *TREE-FORMAT*))
  (cond ((eq *TREE-FORMAT* 'tut)
           (list '|t| `(generic-t pron pers allval pl 1) nil 'empty))
        (t `((form #\t)
...
; ***************************************************************
; *** this looks for adverbial and prepositional modifiers that have been attached
;     to a (nominal) group by the "attach-unlinked" procedure, and checks if it can
;     be attached to a newly inserted trace
(defun move-adverbs-and-prep (data-and-link)
   (let ((sent (first data-and-link))
         (links (second data-and-link))
         newlinks governing-verb)
     (do ((nxtword (first sent) (first locsent))
          (locsent (rest sent) (rest locsent))
          (nxtlink (first links) (first loclinks))
          (loclinks (rest links) (rest loclinks)))
         ((null nxtword)
           (list sent (reverse newlinks)))
         (cond ((or (and (eq (third nxtlink) 'attach-unlinked)
                         (or (lab-subsumes 'ADVB-RMOD (second nxtlink))
                             (lab-subsumes 'PREP-RMOD (second nxtlink))))
                    (eq (third nxtlink) 'final-end))
                 (setq governing-verb (climb-up-to-trace nxtlink sent links))
                 (cond ((null governing-verb)
                          (setq newlinks (cons nxtlink newlinks)))
                       (t (setq newlinks (cons (list governing-verb (second nxtlink) 'top-traces)
                                               newlinks)))))
               (t (setq newlinks (cons nxtlink newlinks)))))))
 
; ***************************************************************
; *** this climbs the tree starting from the word whose link up is startlink
;     the climbing stops when a trace is found, inserted by "insert-top-trace"
;     (success) or when a word which is not a noun, preposition, article or
;     adjective of type demons, deitt, indef is found (failure)
(defun climb-up-to-trace (startlink sent links)
  (let (found parent parlink seenlinks pointer-up)
    (do ()
        (found 
          (cond ((eq found 'fail) nil)
                (t found)))
       (setq pointer-up (first startlink))
       (cond ((or (null pointer-up)
                  (eq 0 pointer-up)
                  (member pointer-up seenlinks))
                (setq found 'fail))
             (t (setq seenlinks (cons pointer-up seenlinks))
                (multiple-value-setq (parent parlink)
                      (find-a-line-split `(position ,pointer-up) sent links))
                (cond ((and (is-a-synt-trace? parent)
                            (eq (get-synt-categ parent) 'VERB)
                            (eq (third parlink) 'top-traces))
                         (setq found (get-synt-numb parent)))
                      ((or (memq (get-synt-categ parent) '(NOUN PREP ART))
                           (and (eq (get-synt-categ parent) 'ADJ)
                                (memq (get-synt-type parent) '(INDEF DEITT DEMONS))))
                         (setq startlink parlink))
                      (t (setq found 'fail))))))))
