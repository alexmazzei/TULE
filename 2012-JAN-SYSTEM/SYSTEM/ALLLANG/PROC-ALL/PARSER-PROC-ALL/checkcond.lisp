			       ...
			       (t nil))))
                     ((eq depcat 'NOUN)
                        (and (neq *SYSTEM-CONTEXT* 'atlas)
                             (eq 'sing (get-synt-number dep))
           ; *** the expression must be "that day" or "the last day", but not "the day"
                             (or (null (find-cf-deps 
                                            (get-synt-numb (first modifs)) 
                                            allines allabs))
                                 (eq 'ADJ (get-synt-categ dep)))
   ...
   ; *** if the verb in question has a parent and that parent is a modal
       ...
       ; *** retrieve all its RMOD dependents
           ...
           ; *** and retrieve the unmarked ones
                     ...
                     (cond ((or (memq nxtcateg '(ART NOUN))
                                (and (eq nxtcateg 'ADJ)
                                     (memq (get-synt-type nxtline) '(INDEF DEITT DEMONS)))
