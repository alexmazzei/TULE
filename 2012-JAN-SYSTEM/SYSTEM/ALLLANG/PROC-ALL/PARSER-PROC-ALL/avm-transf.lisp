...
(defun file-reshuffle-tree (flatfname actfname)
  (declare (special *print-level* *print-length*))
   ...
   (with-open-file (flatavmport flatfname :direction :input
    ...
    ; *** the full avm is output now, since it is used as input by the semantic interpreter
    ;     the simplified form will be output after semantic annotation (in seminterp)
...
(defun from-tut-to-flatavm (prsfile favfile)
           ...
           (cond ((or (string= (string-trim '(#\Space #\Tab #\Return) line) "")
                 ...
                 (t (setq iline (interp-newtb-line line))
                    (let ((position (get-newtb-numb iline))
                          ...
                          value corefinfo avmline hour minute second grade)
 ; *** collection of the syntactic infos
   ;(format t "From tut to flatavm ***** ~% lemma: ~a~%" lemma)
   ;(break "")
                        (setq avmsynt `((lemma ,lemma) (cat ,categ)))
                        (cond ((memq categ '(NOUN ADJ ART PRON CONJ VERB))
                                (setq avmsynt
                                     (append avmsynt 
                        ...
                        (cond ((and (eq categ 'ADJ) (eq (get-newtb-type iline) 'POSS))
                                (setq avmsynt
                                     (append avmsynt 
                                         `((possgend ,(get-newtb-possgender iline))
                                           (possnumb ,(get-newtb-possnumber iline)))))))
                        (cond ((and (eq categ 'ADJ) (eq (get-newtb-type iline) 'QUALIF))
                                (setq grade (get-newtb-adjgrade iline))
                                (cond ((not (null grade))
                                         (setq avmsynt (append1 avmsynt `(grade ,grade)))))))
                        (cond ((eq categ 'NOUN)
                                      ...
                                      (t (setq avmsynt
                                            (append avmsynt 
                                                `((person ,(get-newtb-person iline))
                                                  (number ,(get-newtb-number iline)))))))))
                        (cond ((eq categ 'PRON)
                                (cond ((eq (get-newtb-type iline) 'PERS)
                                         (setq avmsynt 
                                             (append1 avmsynt `(person ,(get-newtb-person iline))))))
                                (cond ((eq (get-newtb-type iline) 'POSS)
                                         (setq avmsynt 
                                             (append1 avmsynt `(posspers ,(get-newtb-possperson iline)
                                                                possnumb ,(get-newtb-possnumber iline))))))
 ...
 ; *** collection of the semantic infos
                        (cond ((eq categ 'ADV) (setq semtype (get-newtb-type iline)))
                              (t (setq semtype (get-newtb-semtype iline))))
   ;(format t "From tut to flatavm ***** ~% semtype: ~a~%" semtype)
   ;(break "")
...
; *******************************************************************
; *** general function for setting feature values of the head of a tree
(defun set-actavm-featval (feature value tree)
   (let ((treehead (get-actavm-head tree)) newhead found)
       (do ((nxtfeat (first treehead) (first treehead))
            (treehead (rest treehead) (rest treehead)))
           ((or (null nxtfeat) found)
              (cond ((not found)
                       (break "avm-transf: set-actavm-featval")))
              `((head ,(append newhead (cond ((null nxtfeat) nil)
                                             (t (list nxtfeat))) treehead))
                (dependents ,(get-actavm-dependents tree))))
           (cond ((eq (first nxtfeat) feature)
                    (setq newhead (append1 newhead (list feature value)))
                    (setq found t))
                 (t (setq newhead (append1 newhead nxtfeat)))))))
...
(defun get-actavm-featval (feature tree)
  (case feature
      ...
      (gender (get-actavm-headgender tree))
      (number (get-actavm-headnumber tree))
...
(defun get-actavm-headsemtype (avmtree)
   (first (leggi (get-actavm-headsem avmtree) 'semtype)))

(defun get-actavm-headthemgrid (avmtree)
   (first (leggi (get-actavm-headsem avmtree) 'thematic-grid)))

(defun get-actavm-headthemrole (avmtree)
   (first (leggi (get-actavm-headsem avmtree) 'thematic-role)))
...
;     returns not only the found tree, but a list including it plus all
...
(defun mult-find-actavm-dep (labels tree allconjuncts)
 (let (found conjuncts)
   ...
   ; *** if a dependent with the right link is found, then it is ireturned as such (allconjuncts=NIL)
   ;     or all the possible conjoined dependents are returned in a  list (allconjuncts=T)
          (cond ((null allconjuncts) found)
                (t (setq conjuncts (find-all-conjuncts found))
                   (cons (first conjuncts) 
                         (mapcar #'(lambda (x) (find-actavm-dep 'COORD2ND x)) (rest conjuncts))))))
      (setq found (find-actavm-descendant '(link) (list nxtlab)
                                   (get-actavm-deps-with-traces tree t))))))

; *******************************************************************
; *** the optionals enable this function to be used both for building
;     "restrictions", that include a restriction type and an upper tree
;     and simple conjuncts, which are just trees
; *** it returns all subtrees headed by the coordinating conjunctions
;     (the first one is the top subtree of the restriction. i.e. not a
;     coordination-headed subtree
;     Tree[John, Bill and Mary] --> 
;           (Tree[John, Bill and Mary] Tree[, Bill and Mary] Tree[and Mary])
(defun find-all-conjuncts (tree)
 (let (restrs)
  (do ((nxtconj tree (find-bq-conjunct nxtconj)))
      ((null nxtconj) 
         ; (format t "Exiting find-all-conjuncts; restrs= ~a ~% ****** ~a~%" 
         ;    (first restrs) (second restrs))
         ; (break "")
          (reverse restrs))
     ; (format t "In find-all-conjuncts; nxtconj= ~a ~%" nxtconj)
     ; (break "")
      (setq restrs (cons nxtconj restrs)))))
...
;     prep
(defun find-obj-all-conjuncts (subtree)
   (cond ((eq (get-actavm-headlexmean subtree) '--about-relation)
            (find-all-conjuncts (find-actavm-dep 'PREP-ARG subtree)))
         (t (find-all-conjuncts subtree))))

; *******************************************************************
; *** this checks if, among the dependents of the head of tree, there is a
;     coordinating conjunction, and returns the subtree of that coordination
;     Tree [John, Bill and Mary] --> Tree [, Bill and Mary]
  ...
  (let ((deps (get-actavm-deps-with-traces tree)) found)
    (do ((nxtdep (first deps) (first deps))
        ...
        (cond ((equal nxtdep '(#\#)) nil)
              ((and (lab-subsumes 'COORD (get-actavm-headlink nxtdep))
...
(defun list-actavm-dep (dep avmtree indent)
  ...
  (cond ((or (eq dep #\#) (equal dep '(#\#)))
         ...
         ;           (setq semcomp meaning)))
           (setq semcomp (get-actavm-headsem avmtree))
