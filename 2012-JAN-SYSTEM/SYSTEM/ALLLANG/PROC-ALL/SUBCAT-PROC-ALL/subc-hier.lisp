...
;     (DEPENDENT)
(defun get-lab-ancestors-old (label &optional all)
   (cond ((memq label '(NOFUNCTION RMOD ARG APPOSITION VERB-ARG))
         ...
         (t (cons label
                (merge-flatten (mapcar #'(lambda (x) (get-lab-ancestors-old x all))
...
(defun get-lab-ancestors (label &optional all)
   (cond ((or (memq label '(NOFUNCTION RMOD ARG APPOSITION 
                        VERB-SUBJ VERB-OBJ VERB-ARG))
              (memq 'VERB-INDCOMPL (get label 'more-gen-labels)))
   ; *** the last condition because indirect complements (e.g verb-indcompl-loc
   ;     and verb-indobj) are different complements and should not be merged
           (cond (all 
                   (cond ((eq label 'RMOD)
                            '(RMOD FUNCTION))
                         ((memq label '(VERB-SUBJ VERB-OBJ))
                            (list label 'VERB-ARG 'ARG 'FUNCTION))
                         ((memq 'VERB-INDCOMPL (get label 'more-gen-labels))
                            (list label 'VERB-INDCOMPL 'VERB-ARG 'ARG 'FUNCTION))
                         ((eq label 'VERB-ARG)
                            '(VERB-ARG ARG FUNCTION))
                         ((eq label 'ARG)
                            '(ARG FUNCTION))
                         ((eq label 'APPOSITION)
                            '(APPOSITION FUNCTION))
                         (t '(NOFUNCTION))))
   ; *** if all is false, but the current label is a dependent of 'verb-indcompl,
   ;     it must be returned as top ancestor
                 ((or (memq 'VERB-INDCOMPL (get label 'more-gen-labels))
                      (memq label '(VERB-SUBJ VERB-OBJ)))
                    (list label))
                 (t nil)))
	 ((null label)
            (exception 'parse-error "PROC/subc-hier: get-lab-ancestors 1"))
         ((eq label 'DEPENDENT)
           (cond (all '(DEPENDENT))
	         (t (exception 'parse-error
                            "PROC/subc-hier: get-lab-ancestors 2"))))
