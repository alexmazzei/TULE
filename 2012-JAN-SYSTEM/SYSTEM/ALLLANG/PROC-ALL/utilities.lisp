...
; *** as assoc, but the search key can be a list
(defun list-assoc (alist keylist)
  (cond ((null alist) nil)
        ((equal (first (first alist)) keylist) (first alist))
        (t (list-assoc (rest alist) keylist))))

; *** as leggi, but the search key can be a list
(defun list-leggi (alist keylist)
  (cdr (list-assoc alist keylist)))

; *** as leggi, alist can include multiple occurences of the search key; returns a list
;     (mult-leggi '((a b) (c d) (a e) (f g)) 'a) ---> (b e)
(defun mult-leggi (alist value)
  (cond ((null alist) nil)
        ((equal (first (first alist)) value) 
            (cons (rest (first alist)) (mult-leggi (rest alist) value)))
        (t (mult-leggi (rest alist) value))))
...
;************************************************************************
; *** eliminates the nils and the (nil) from a list
(defun dropnil2 (l)
  (cond ((null l) nil)
        ((or (null (car l))
             (equal (car l) '(nil)))
           (dropnil2 (cdr l)))
        (t (cons (car l) (dropnil2 (cdr l))))))
...
;************************************************************************
; *** deletes the duplicates from a list (not remdup! this is for grep)
(defun elimdup (list)
  (cond ((null list) nil)
