...
(defun postagger
  ...
  (let ((frase-in (mapcar #'first phr-scanner+analyser))
     ...
     (setq finalres (extractfraseinfos frase-in (first disamb-res)))
     finalres))
    ...
    ;     the disambiguation fails (error flag in the output)
              (cond ((null disword)
                      (cond ((eq 'yes (prendi (first savelocut) 'proper))
                               (setq wrdchoice '(($SAME CAT NOUN PROPER YES)))
                               (setq reason '("unknown-proper")))
                            (t (setq wrdchoice '(($SAME CAT NOUN PROPER YES)))
                              ;!!!  (setq wrdchoice "Exists only as part of a locution")
                               (setq reason '("morphological failure")))))
...
;     to which, here is added for completely random choices
...
;            (extract-interp disword 'cat firstcateg t 'all)
;                   but also to extract something in case the word is "all-proper" (?)
...
(defun sld-prendi (el feat)
  ; *** for composite word, the first part is used
  (cond ((not (atom (first el)))
           (setq el (first el))))
...
;********************************************************************
; *** builds the names of the categories of compound words (e.g. prep+arg)
...
; *** inpcat may have the form NOUN (i.e. an atom specifying the category of
;     the current word, which may have more than one) or 
;     (VERB (MOOD IND TENSE PRES))
;     in case the item on which the agreement must be checked must satisfy further
;     constraints
(defun prevcat-agr (categs inpcat)
  (declare (special disword rev-iniz-frase))
  (let (thiscat constr thisinterp thisnumber thisgender newcondit)
     (cond ((atom inpcat)
              (setq thiscat inpcat))
           (t (setq thiscat (first inpcat))
              (setq constr (second inpcat))))
     (setq thisinterp (filter-interp (extract-interp disword 'cat thiscat t 'last) constr))
     (setq thisnumber (mapcar #' (lambda (x) (prendi (without-lemma x) 'number)) thisinterp))
     (setq thisgender (mapcar #' (lambda (x) (prendi (without-lemma x) 'gender)) thisinterp))
...
;********************************************************************
; *** interp is a list of word entries (e.g. (QUESTO CAT ADJ TYPE DEMONS GENDER M ...))
;     constr is a list of feature-name fetaure-value (e.g. (GENDER M TYPE DEMONS))
(defun filter-interp (interps constr)
  (let (result)
     (cond ((null constr) interps)
           (t (dolist (nxtinterp interps)
                  (cond ((or (equal (second constr)
                                    (prendi (without-lemma nxtinterp) (first constr)))
                             (and (eq (first constr) 'proper)
                                  (null (prendi (without-lemma nxtinterp) 'proper))))
                           (setq result (append1 result nxtinterp)))))
              (filter-interp result (rest (rest constr)))))))
...
;     "cond" si stampa la parentesi aperta e la forma normalizzata e la
                     ...
                     (cond ((eq (prendi s-word 'form) 'clitic)
                           ...
                           ((eq type 'poss)
                              (let ((ppers (prendi s-word 'posspers))
                                 ...
                                 (cond ((not (null ppers))
                                          (setq tempout (append1 tempout (concat 'p- ppers)))))
                                 (cond ((not (null pgend))
                                          (setq tempout (append1 tempout (concat 'p- pgend)))))
                                 (cond ((not (null pnumb))
                                          (setq tempout (append1 tempout (concat 'p- pnumb)))))))
                           ((eq type 'ordin)
                               (setq tempout (append2 tempout (prendi s-word 'value))))))
                   ((eq categ 'adj)
                     (cond ((eq type 'poss)
           ...
           ((eq out-type 'sent)
              (setq syntempout
                  (cond ((eq categ 'verb)
                         ...
                         ((or (memq categ '(noun art predet))
                              (and (eq categ 'adj) (eq grade 'pos)))
                           `((lemma ,root) (cat ,categ) (type ,type)
                             (gender ,gender) (number ,number)))
                         ((and (eq categ 'adj) (neq grade 'pos))
                           `((lemma ,root) (cat ,categ) (type ,type)
                             (gender ,gender) (number ,number) (grade ,grade)))
                         ((and (eq categ 'pron) (or (null grade) (eq grade 'pos)))
                           `((lemma ,root) (cat pron) (type ,type)
                             (gender ,gender) (number ,number) (person ,person)
                             (case ,(concatl
                                      (put-separator '+ (prendi s-word 'case))))))
                         ((and (eq categ 'pron) (not (null grade)) (neq grade 'pos))
                           `((lemma ,root) (cat pron) (type ,type)
                             (gender ,gender) (number ,number) (person ,person) (grade ,grade)
                             (case ,(concatl
                                      (put-separator '+ (prendi s-word 'case))))))
               ...
               (setq semtempout
                   (cond ((eq categ 'num)
                         ...
                         ((and (eq categ 'conj) (eq type 'coord))
                           `((semtype ,(cond ((atom semtype) semtype)
                                             (t (concatl (put-separator '+ semtype)))))))
                         ((and (eq categ 'prep) (eq type 'poli))
                           `((semtype ,semtype)))
                         ((eq categ 'date)
                     ...
                     ((eq categ 'pron)
                        (cond ((eq (prendi s-word 'form) 'clitic)
                              ...
                              ((eq type 'poss)
                                 ...
                                 (cond ((not (null ppers))
                                          (setq syntempout 
                                              (append1 syntempout `(posspers ,(concat 'p- ppers))))))
                                 (cond ((not (null pgend))
                                          (setq syntempout 
                                              (append1 syntempout `(possgend ,(concat 'p- pgend))))))
                                 (cond ((not (null pnumb))
                                          (setq syntempout 
                                              (append1 syntempout `(possnumb ,(concat 'p- pnumb))))))))
                              ((eq type 'ordin)
